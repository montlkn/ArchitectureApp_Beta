...
export default function App() {
    ...
    const handleCapture = async (base64Image, location) => {
        ...
        try {
            ...
            const { data, error } = await supabase.rpc('match_building_by_landmark', { landmark_name: "some landmark", user_lat: location.latitude, user_long: location.longitude });
            if (error) throw new Error(`Database search failed: ${error.message}`);
            if (!data || data.length === 0) throw new Error(`We could not identify this landmark.`);

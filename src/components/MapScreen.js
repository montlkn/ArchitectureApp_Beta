    ...
    return (
        <Marker coordinate={{latitude: marker.latitude, longitude: marker.longitude}} onPress={onPress} tracksViewChanges={false}>
            <View style={externalStyles.mapMarkerContainer}>
                ...
                <View style={[externalStyles.mapMarkerPin, { backgroundColor: color }]} />
            </View>
        </Marker>
    );
});
...
const MapScreen = ({ navigation }) => { // FIX: Use the 'navigation' prop
    ...
    return (
        <View style={{flex: 1}}>
            ...
            >
                {markers.map(marker => marker.is_cluster ?
                    <ClusterMarker key={marker.id} pointCount={marker.point_count} coordinate={{latitude: marker.latitude, longitude: marker.longitude}} onPress={() => handleMarkerPress(marker)} /> :
                    <BuildingMarker key={marker.id} marker={marker} zoom={zoom} onPress={() => handleMarkerPress(marker)} />
                )}
            </MapView>
...
const styles = StyleSheet.create({
    clusterContainer: {
        justifyContent: 'center', alignItems: 'center', backgroundColor: 'rgba(23, 107, 135, 0.9)',
